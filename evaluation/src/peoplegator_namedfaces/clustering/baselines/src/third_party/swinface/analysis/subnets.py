...
class ConvLayer(torch.nn.Module):
    ...
    def __init__(self, in_chans=768, out_chans=512, conv_mode="normal", kernel_size=3):
        ...
        if conv_mode == "normal":
            self.conv = nn.Conv2d(in_chans, out_chans, kernel_size, stride=1, padding=(
                kernel_size-1)//2, bias=False)
        elif conv_mode == "split":
            self.convs = nn.ModuleList()
            for j in range(len(in_chans)):
                conv = nn.Conv2d(in_chans[j], out_chans[j], kernel_size, stride=1, padding=(
                    kernel_size-1)//2, bias=False)
...
class LANet(torch.nn.Module):
    def __init__(self, in_chans=512, reduction_ratio=2.0):
        ...
        self.conv1 = nn.Conv2d(self.in_chans, self.mid_chans, kernel_size=(
            1, 1), stride=(1, 1), bias=False)
        self.conv2 = nn.Conv2d(self.mid_chans, 1, kernel_size=(
            1, 1), stride=(1, 1), bias=False)
...
def MAD(x, p=0.6):
    ...
    mask1 = torch.cat([torch.randperm(C).unsqueeze(dim=0)
                      for j in range(B)], dim=0).cuda()
...
class LANets(torch.nn.Module):
    ...
    def __init__(self, branch_num=2, feature_dim=512, la_reduction_ratio=2.0, MAD=MAD):
        ...
        for i in range(branch_num):
            self.LANets.append(
                LANet(in_chans=feature_dim, reduction_ratio=la_reduction_ratio))
...
class FeatureAttentionNet(torch.nn.Module):
    def __init__(self, in_chans=768, feature_dim=512, kernel_size=3,
        ...
        if self.spatial_attention == "CBAM":
            ...
            self.spatial_attention = LANets(
                branch_num=la_branch_num, feature_dim=feature_dim)
