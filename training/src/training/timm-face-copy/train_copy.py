...
# Add the dataset repository to the python path
dataset_repo_path = "/storage/brno2/home/karel_srna/KNN/repos/knn-repo/neural-face-identifier/datasets/src"
if dataset_repo_path not in sys.path:
    sys.path.append(dataset_repo_path)

# Now you can import your specific classes
from datasets import PeopleGatorDataset, PeopleGatorPairDataset
...
def get_parser():
    ...
    # added args
    parser.add_argument("--train_peoplegator_jsonl", type=Path, default=None)
    parser.add_argument("--train_peoplegator_images_root", type=Path, default=None)
    parser.add_argument("--val_peoplegator_jsonl", type=Path, default=None)
    parser.add_argument("--val_peoplegator_images_root", type=Path, default=None)
    parser.add_argument("--skip_insightface", action="store_true", help="Skip InsightFace evaluation")
...
if __name__ == "__main__":
    ...
    # --- DATA LOADING LOGIC ---
    if args.train_peoplegator_jsonl and args.train_peoplegator_images_root:
        # Use PeopleGator as the training set
        if is_rank0:
            print("Loading PeopleGator training dataset...")
        train_ds = PeopleGatorDataset(
            args.train_peoplegator_jsonl, 
            args.train_peoplegator_images_root,
            transform=None  # PeopleGatorDataset has built-in transforms
        )
        train_size = len(train_ds)
        if is_rank0:
            print(f"Train dataset: {train_size:,} images")
            print(f"{args.total_steps / (train_size // batch_size):.2f} epochs")
        
        dloader = DataLoader(
            train_ds, 
            batch_size=batch_size // args.grad_accum,
            shuffle=True, 
            num_workers=args.n_workers,
            pin_memory=True
        )
        # Create an infinite generator to match your 'next(dloader)' pattern
        def infinite_loader(loader):
            while True:
                for batch in loader:
                    yield batch
        dloader_iter = infinite_loader(dloader)
    else:
        # Fallback to the original InsightFaceBin loader
        dloader_iter, train_size = create_train_dloader(
            args.ds_path,
    ...
    # Initialize validation dataloader (only used if is_rank0)
    pg_dloader = None
    if args.val_peoplegator_jsonl and args.val_peoplegator_images_root:
        if is_rank0:
            print("Initializing fixed PeopleGator Validation Set...")
            pg_val_base = PeopleGatorDataset(args.val_peoplegator_jsonl, args.val_peoplegator_images_root)
            # The seed 42 here ensures pairs are identical across all evaluation runs
            pg_pair_ds = PeopleGatorPairDataset(pg_val_base, seed=42)
            pg_dloader = DataLoader(pg_pair_ds, args.batch_size, num_workers=args.n_workers)
    
    # Setup validation dataset paths for InsightFace
    ...
    while step < args.total_steps:
        ...
        for _ in range(args.grad_accum):
            images, labels = next(dloader_iter)
        ...
        if is_rank0 and step % args.eval_interval == 0:
            ...
            # Evaluate on PeopleGator if available
            if pg_dloader is not None:
                all_labels = []
                all_scores = []

                for imgs1, imgs2, labels in tqdm(pg_dloader, desc=f"Eval PeopleGator Step {step}"):
                    all_labels.append(labels.cpu().numpy())
                    with torch.no_grad(), torch.autocast("cuda", amp_dtype, amp_enabled):
                        # Using ema (Exponential Moving Average) is best for ViT validation
                        embs1 = ema(imgs1.to("cuda")).float()
                        embs2 = ema(imgs2.to("cuda")).float()
                        
                        # Use cosine similarity (dot product of normalized embeddings)
                        # ArcFace/CosFace models expect this
                        dist = (embs1 * embs2).sum(1).cpu().numpy()
                        all_scores.append(dist)

                all_labels = np.concatenate(all_labels)
                all_scores = np.concatenate(all_scores)

                # This will now be 100% reproducible because the pairs never change
                acc = kfold_accuracy(all_labels, all_scores)
                wandb.log({"acc/peoplegator": acc}, step=step)
        ...
        if is_rank0:
            ...
            if step % args.eval_interval == 0:
                ...
                # Only run InsightFace if not skipped
                if not args.skip_insightface and len(val_ds_paths) > 0:
